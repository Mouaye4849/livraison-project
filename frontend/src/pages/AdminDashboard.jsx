    ...
    if (active && payload && payload.length) {
        return (
            <div className="bg-white border border-gray-200 rounded-xl shadow-lg px-4 py-2.5 text-sm">
                ...
                <p className="text-indigo-600 font-bold mt-0.5">{payload[0].value} colis</p>
            </div>
        );
    }
    return null;
...
/* ─── STATUS CONFIG ─── */
const STATUS_CONFIG = {
    EN_COURS: { label: "En cours", bg: "bg-amber-50", text: "text-amber-700", dot: "bg-amber-400", border: "border-amber-200" },
    ACCEPTE: { label: "Accepté", bg: "bg-blue-50", text: "text-blue-700", dot: "bg-blue-400", border: "border-blue-200" },
    LIVRE: { label: "Livré", bg: "bg-emerald-50", text: "text-emerald-700", dot: "bg-emerald-400", border: "border-emerald-200" },
    ANNULE: { label: "Annulé", bg: "bg-rose-50", text: "text-rose-700", dot: "bg-rose-400", border: "border-rose-200" },
    PUBLIE: { label: "Publié", bg: "bg-violet-50", text: "text-violet-700", dot: "bg-violet-400", border: "border-violet-200" },
};
...
export default function AdminDashboard() {
    ...
    /* ─── CARD DATA ─── */
    const cards = [
        { title: "Total colis", value: stats.totalColis, icon: Package, idx: 0 },
        { title: "Trajets actifs", value: stats.totalTrajets, icon: Truck, idx: 1 },
        { title: "Revenus", value: `${revenue} MRU`, icon: DollarSign, idx: 2 },
        { title: "Transactions", value: transactions, icon: CreditCard, idx: 3 },
        { title: "Croissance", value: `${croissance}%`, icon: TrendingUp, idx: 4 },
    ];
            ...
            <div className="p-4 md:p-8 space-y-6 bg-gray-50 min-h-screen font-sans">
                ...
                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 fade-up">
                    <div>
                        ...
                        <p className="text-gray-400 text-sm mt-0.5">
                            Suivi global de la plateforme en temps réel
                        </p>
                    </div>
                    ...
                    <div className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-sm font-semibold border
                        ...
                        {isPositive ? "+" : ""}{croissance}% cette semaine
                    </div>
                </div>
                ...
                <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 fade-up">
                    <div className="flex items-center justify-between mb-5">
                        <div>
                            <h2 className="text-base font-semibold text-gray-900">Activité hebdomadaire</h2>
                            <p className="text-xs text-gray-400 mt-0.5">Nombre de colis par jour</p>
                        </div>
                ...
                <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden fade-up">
                    <div className="px-6 py-4 border-b border-gray-50 flex items-center justify-between">
                        <h2 className="text-base font-semibold text-gray-900">Liste des colis</h2>
                    ...
                    {!loading && filtered.length === 0 && (
                        <div className="flex flex-col items-center justify-center py-16 px-6 text-center">
                            ...
                            <p className="text-gray-700 font-medium">Aucun colis trouvé</p>
                            <p className="text-gray-400 text-sm mt-1">
                                {filter === "TOUS"
                                    ? "Il n'y a encore aucun colis enregistré."
                            ...
                            <table className="w-full text-sm">
                                <thead>
                                    <tr className="text-left bg-gray-50/60 border-b border-gray-100">
                                        <th className="px-6 py-3 text-xs font-semibold text-gray-400 uppercase tracking-wider">Commande</th>
                                        ...
                                        <th className="px-4 py-3 text-xs font-semibold text-gray-400 uppercase tracking-wider">Prix</th>
                                    </tr>
                                </thead>
                                    ...
                                    {filtered.map((c) => {
                                        ...
                                        return (
                                            <tr key={c.id} className="row-hover transition-colors duration-150">
                                                <td className="px-6 py-3.5 font-mono font-medium text-gray-800 text-xs">
                                                    {formatId(c.id)}
                                                </td>
